...
def _make_gbm_methylation(seed: int = 3) -> SyntheticView:
    ...
    Manuscript stats: DI ≈ 1.00 (random-like overlap between variance-ranked
    and SHAP-ranked features), rho = 0.02.

    Design:
      - n = 136 GBM samples (subtype classification)
      - p = 8,000 CpG probes
      - Noise block (0–15%): pure high-variance noise, zero class signal.
        Completely fills the top-10% variance window, so TopVar selects
        features with essentially no discriminative information.
      - Signal block (15–25%): moderate-low variance with genuine class
        differences; never enters the top-10% variance window.

    Result: η_ES ≈ 0.29, VSA < 0 → RED_HARMFUL consistently.
    This is coherent with DI ≈ 1.00: variance filtering provides a random
    sample of important features (neither systematically helpful nor harmful
    in SHAP-overlap terms), while the VAD shows the top-var features are
    actively depleted of signal.
    """
    rng = np.random.default_rng(seed)
    n, p = 136, 8_000
    n_classes = 4
    ...
    # Noise block: 15% of features (1200 probes) at high variance, no signal.
    # 1200 > top-10% window (800), so the entire top-10% is noise-only.
    n_noise = int(p * 0.15)
    X[:, :n_noise] *= 5.0

    # Signal block: features 15–25% of probes, genuine class differences but
    # kept at low variance so they never compete for top-10% spots.
    n_sig_start = n_noise
    n_sig_end   = int(p * 0.25)
    for c in range(n_classes):
        mask = (y == c)
        X[mask, n_sig_start:n_sig_end] += c * 1.0
    X[:, n_sig_start:n_sig_end] *= 1.3
    ...
    return SyntheticView(
        ...
        manuscript_rho=0.02,
        description=(
            "TCGA-GBM methylation: the high-variance features are methylation "
            "noise (η_ES ≈ 0.29, VAD = RED_HARMFUL). The manuscript DI ≈ 1.00 "
            "means variance-ranked and SHAP-ranked feature sets overlap at "
            "chance level — variance filtering provides a random sample of "
            "important features. Both VAD and DI consistently indicate that "
            "variance filtering is unreliable for this view. Treated as a "
            "sensitivity analysis in the manuscript because GBM subtype signal "
            "is weaker than in the primary cohorts."
        ),
    )
